...
const kpis = [
  { label: 'PO Value', value: '14.6M' },
  { label: 'Open POs', value: '9' },
  { label: 'Receipts', value: '18' },
...
const details = ref([
  { supplier: 'BRG Factory', pos: '14', received: '12', spend: '11.2M', leadTime: '25d', onTime: '92%' },
  { supplier: 'PowerCell Co.', pos: '5', received: '5', spend: '3.4M', leadTime: '14d', onTime: '95%' },
  { supplier: 'Pak Logistics', pos: '8', received: '8', spend: '0.9M', leadTime: '5d', onTime: '88%' }
])
</script>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0">
      <div>
        <div class="text-[10px] text-gray-500 mb-1">
          Super Admin / Analytics / Reports / <span class="font-bold text-gray-800">Procurement Report</span>
        </div>
        <h1 class="text-[32px] tracking-tight font-bold text-gray-900">Procurement Report</h1>
        <p class="text-sm text-gray-500 mt-1">Operational procurement report with branch and date filters.</p>
      </div>
        ...
        <div class="h-44 flex items-center justify-center p-2 bg-[#fcfdfd] rounded-lg">
          <svg class="w-full h-full" viewBox="0 0 400 120" preserveAspectRatio="none">
            <polyline
              ...
              points="10,95 70,88 140,80 210,74 280,68 350,56 390,48"
            />
          </svg>
        </div>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="text-[11px]">
            ...
            >
              ...
              <td class="px-6 py-4 text-gray-600 font-medium">{{ item.pos }}</td>
              <td class="px-6 py-4 text-gray-600 font-medium">{{ item.received }}</td>
              <td class="px-6 py-4 text-gray-900 font-bold">{{ item.spend }}</td>
              <td class="px-6 py-4 text-gray-600 font-medium">{{ item.leadTime }}</td>
              <td class="px-6 py-4 text-gray-900 font-bold">{{ item.onTime }}</td>
            </tr>
          </tbody>
        </table>
      </div>
